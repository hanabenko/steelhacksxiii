...
const features=elements.filter(e=>e.type==='way'&&(e.tags.building||e.tags['building:part']||e.tags.highway||e.tags.landuse==='grass'||e.tags.leisure==='garden')).map(e=>({id:e.id,tags:e.tags,points:e.refs.map(r=>nodes.get(r)).filter(Boolean).map(local)})).filter(e=>e.points.some(within));
