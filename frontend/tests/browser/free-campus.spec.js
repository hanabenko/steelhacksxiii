...
test('map has all twelve placement footprints; mouse navigation keeps the selected upgrade',async({page})=>{
 ...
 await expect(page.locator('#budget-hud')).toHaveText('$64,000');await expect(page.locator('#network-design-status')).toContainText('3 of 3 sites edited');await page.locator('#test-design').click();await expect(page.locator('#result-status')).toHaveText('100 RUNS',{timeout:15000});
});
...
test('hover feedback distinguishes duplicate infrastructure from insufficient funds',async({page})=>{
 ...
 if(await page.locator('button[data-panel="design"]').getAttribute('aria-expanded')!=='true')await switchPanel(page,'design');await page.locator('.keyboard-placement>summary').click();await page.locator('[data-tool="diet"]').click();
...
test('matching navy right panels toggle independently and keep navigation visible',async({page})=>{
 await page.goto('/');await enterGame(page);await expect(page.locator('#design-panel')).not.toBeVisible();await expect(page.locator('.tools-panel')).toHaveCount(0);
 let previous;
 for(const name of ['design','simulation','results']){
  await switchPanel(page,name);const panel=page.locator('#'+name+'-panel');await expect(panel).toBeVisible();await expect(page.locator('.navigation-panel')).toBeVisible();
 ...
 await switchPanel(page,'design');await expect(page.locator('[data-tool="signal"]')).toHaveCount(0);await expect(page.locator('[data-tool="shelter"]')).toBeVisible();
...
test('intersection arrows cycle all sites and preserve the open build panel',async({page})=>{
 await page.goto('/');await enterGame(page);await page.locator('#view-top').click();await switchPanel(page,'design');
...
test('design map arrows and rotation remain usable in street view',async({page})=>{
 await page.goto('/');await enterGame(page);await expect(page.locator('.design-camera')).not.toBeVisible();
 await page.locator('#view-top').click();await switchPanel(page,'design');
 await page.getByRole('button',{name:'Jump to next intersection',exact:true}).click();
 const canvas=page.locator('#scene canvas');await expect(canvas).toHaveAttribute('data-viewed-intersection','pitt-fifth-bigelow');
 await page.locator('#design-rotation').fill('120');await expect(canvas).toHaveAttribute('data-camera-rotation','120');
 await page.locator('button[data-navigation="street"]').click();await expect(page.locator('#design-panel')).toBeVisible();
 await page.locator('#design-rotation').fill('90');await expect(canvas).toHaveAttribute('data-navigation','street');await expect(canvas).toHaveAttribute('data-camera-rotation','90');
