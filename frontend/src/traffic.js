...
export function createTraffic(features,count=42){
  const routes=campusRoutes(features);
  const vehicles=Array.from({length:count},(_,i)=>{const route=routes[i%routes.length],rank=Math.floor(i/routes.length)+(i%routes.length===7?.5:0);return{route,s:(12+rank*62)%route.length,speed:0,desired:7.8+(i*17%19)/10,enabled:true,braking:false,pose:poseAt(route,(12+rank*62)%route.length)};});
  function update(dt,signals,upgrades=[]){
    ...
    while(dt>0){const h=Math.min(dt,1/30);dt-=h;const snapshots=vehicles.map(v=>({...v,pose:poseAt(v.route,v.s)}));
      vehicles.forEach((v,i)=>{if(!v.enabled)return;const old=snapshots[i];let gap=Infinity,target=v.desired;
        ...
        for(const other of snapshots){if(other===old||!other.enabled)continue;const dx=other.pose.x-old.pose.x,dz=other.pose.z-old.pose.z;const along=dx*Math.sin(old.pose.angle)+dz*Math.cos(old.pose.angle),lateral=Math.abs(dx*Math.cos(old.pose.angle)-dz*Math.sin(old.pose.angle));
          if(along>0&&lateral<2.3&&Math.cos(other.pose.angle-old.pose.angle)>.5)gap=Math.min(gap,along-4.5);
        }
        for(const stop of v.route.stops){const distance=stop.s-v.s,state=stop.axis==='x'?signals.penn:signals.cross;if(distance>=0&&(state==='red'||state==='amber'&&distance>v.speed*v.speed/7+2))gap=Math.min(gap,distance+1.5);
        ...
        const acceleration=Math.max(-4.5,Math.min(1.7,(target-v.speed)*1.8));v.braking=acceleration<-.35;v.speed=Math.max(0,v.speed+acceleration*h);v.s+=Math.min(v.speed*h,Math.max(0,gap-1.5));
        if(v.s>=v.route.length){const entry=poseAt(v.route,0);if(!snapshots.some(o=>o.enabled&&Math.hypot(o.pose.x-entry.x,o.pose.z-entry.z)<12)){v.s=0;v.speed=0;}else v.s=v.route.length;}
