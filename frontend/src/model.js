...
import { WEATHER, DEFAULT_CONDITIONS, validateConditions, resolvedHazards } from './scenarios.js';
...
export const TOOLS = [
  ...
  { id:'repair',name:'Road repair',detail:'Repair a pothole or reopen a damaged-road closure on this approach. Match the challenge intersection and approach.',benefit:'Restore the road surface',tradeoff:'Uses the repair budget',cost:costs.allowances.repair,icon:'Route' },
];
...
export function validateSettings(settings) {
  ...
  if(settings.pedestrianDemand!==undefined&&(!Number.isFinite(settings.pedestrianDemand)||settings.pedestrianDemand<0||settings.pedestrianDemand>5000))throw new Error("Invalid pedestrian demand");
...
export function simulate(items, settings = DEFAULT_SETTINGS, seed = 42, {includeTrials=false} = {}) {
  ...
  const effective=resolvedHazards(conditions,items.map(i=>({...i,intersection:i.intersection||settings.siteId||DEFAULT_INTERSECTION})));
  const closures=new Set(items.filter(i=>i.type==='closure').map(i=>i.zone));if(scenarioClosed&&effective.closure)closures.add(conditions.closure.zone);
  ...
  const pedestrianDemand=settings.pedestrianDemand??900;
  const pedestrianCapacity=600*(1-Math.max(0,settings.green-35)*.006);
  const samples = { before: [], after: [] };
  for (let i=0; i<settings.runs; i++) {
    const demand = settings.demand * (0.85 + rng() * .3), noise = .9 + rng() * .2;
    const base = { risk: 12.4 * demand / 800 * noise*weather.risk*(hasPothole?1.2:1)*hazardRisk, speed: observedSpeedMph(rng())*weather.speed*(hasPothole?.8:1)*hazardSpeed, delay: 24 + demand / 90 + Math.max(0,demand-beforeCapacity)*.04+(hasPothole?6:0)+placedPotholes*3, throughput: Math.min(demand, beforeCapacity), pedestrianThroughput:Math.min(pedestrianDemand*(.85+(noise-.9)*1.5),pedestrianCapacity*(scenarioClosed?.85:1)), access: 48 };
    ...
    samples.after.push({ risk: base.risk*riskFactor/(repaired||protectedPothole?1.2:1), speed: Math.max(5, base.speed/(repaired?.8:1)-cross*.8-curb*1.2-diet*2), delay: Math.max(8, base.delay+cross*.9+diet*3/* AV delay effect disabled: -settings.av*.04 */+(35-settings.green)*.16+(Math.max(0,demand-capacity)-Math.max(0,demand-beforeCapacity))*.04-(repaired?6:0)), throughput: Math.min(demand,capacity), pedestrianThroughput:Math.min(pedestrianDemand*(.85+(noise-.9)*1.5),(pedestrianCapacity+cross*100+curb*70)*Math.max(.4,1-closures.size*.15)), access: Math.min(100,48+cross*9+curb*7+bike*3+diet*3+shelter*4-Math.max(0,settings.green-35)*.35) });
  }
...
export function simulateNetwork(items, settings=DEFAULT_SETTINGS, seed=42){
  ...
  for(const phase of ['before','after']){
    combined[phase]={};
    for(const metric of ['risk','speed','delay','throughput','pedestrianThroughput','access']){
      const values=Array.from({length:settings.runs},(_,i)=>sites.reduce((sum,site)=>sum+site.trials[phase][i][metric],0)/(['throughput','pedestrianThroughput'].includes(metric)?1:sites.length));
  ...
  const score=Math.round(Math.min(100,Math.max(0,30+reduction*.65+(after.access.mean-48)*.5-Math.max(0,95-retained))));
  return {before,after,reduction,retained,score,seed,runs:settings.runs,engine:'local-network-surrogate-v1',intersections:sites.map(({trials,...site})=>site),aggregation:'Equal-demand mean across three intersections; vehicle and pedestrian throughput sum intersection passages, not unique travelers. Pedestrian demand and capacity are illustrative assumptions. Shared paired demand trials. No rerouting or queue spillback.'};
}
