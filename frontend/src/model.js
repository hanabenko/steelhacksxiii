...
export function simulate(items, settings = DEFAULT_SETTINGS, seed = 42, {includeTrials=false} = {}) {
  ...
  for (let i=0; i<settings.runs; i++) {
    ...
    const capacity = Math.max(400, 980 - diet*90 - cross*16 - curb*10 + settings.av*1.2 + (settings.green-35)*3);
    const riskFactor = Math.max(.18, 1 - cross*.11 - bike*.08 - curb*.09 - diet*.13 - settings.av*.0015);
    samples.before.push(base);
    samples.after.push({ risk: base.risk*riskFactor, speed: Math.max(15, base.speed-cross*.8-curb*1.2-diet*2), delay: Math.max(8, base.delay+cross*.9+diet*3-settings.av*.04+(35-settings.green)*.16+(Math.max(0,demand-capacity)-Math.max(0,demand-980))*.04), throughput: Math.min(demand,capacity), access: Math.min(100,48+cross*9+curb*7+bike*3+diet*3+shelter*4-Math.max(0,settings.green-35)*.35) });
  }
...
export function exportScenario(items, settings, result) {
  return { schemaVersion:2, intersection:'pitt-campus-network', intersections:INTERSECTIONS.map(({id,name,sourceUrl})=>({id,name,sourceUrl})), createdAt:new Date().toISOString(), costBasis:costs, budget:BUDGET, spent:costOf(items), upgrades:items.map(i=>({...i,intersection:i.intersection||DEFAULT_INTERSECTION})), settings:{...settings}, result, disclaimer:'Uncalibrated frontend surrogate. Not a crash forecast. See README for SUMO integration.' };
}
