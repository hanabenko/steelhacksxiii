...
export function createCollisionPreview(scene,templates){
  ...
  let time=12;
  return{start(x=0,z=0){time=0;root.position.set(x,.25,z);root.visible=true;},update(dt,visible=true){time=Math.min(12,time+dt);root.visible=visible&&time<12;const impact=Math.max(0,time-1.2),approach=Math.max(0,1.2-time);
