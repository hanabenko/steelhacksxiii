export function createWalkthrough({openPanel,getState}) {
  ...
  const steps=[
    {title:'Welcome to your street lab',body:'Watch the signals cycle from green to amber to red. Cars stop at red lights. Left-drag to move around all three sites, right-drag to orbit, or use Top down to plan.',panel:null,target:'.signal-hud'},
    ...
    {title:'Place your first upgrade',body:'Drag the raised crosswalk card onto a blue upgrade silhouette. Or select the card and click a blue shape at any of the three sites. Design also has keyboard approach buttons. This changes your actual design—you can undo it anytime.',panel:'design',target:'[data-tool="crosswalk"]',gate:s=>s.count>0,pending:'Place an upgrade to continue.'},
    {title:'Run a fair comparison',body:'Choose traffic demand and signal timing, then Run simulation. Before and After use the same sampled demand. Under 150 rounds, watch a fast preview; longer runs illustrate 30 rounds while calculating all of them. Playback returns to normal afterward.',panel:'simulation',target:'#run',gate:s=>!!s.result,pending:'Run the simulation to continue.'},
    ...
    {title:'Ask your Street assistant',body:'Use the microphone icon and Street assistant label in the top toolbar. Type a question or choose Record question to use your microphone. Design advice explains your current design, and Speak replies reads answers aloud. Finish the tour, then give it a try.',panel:null,target:'.header .assistant summary'},
  ];
  ...
  function show(index){
    clearHighlight();const assistant=document.querySelector('.assistant');if(assistant)assistant.open=false;step=index;const data=steps[step];openPanel(data.panel);
    ...
    if(step===3&&document.body.dataset.mode==='game')$('#tour-body').textContent='The original street baseline is ready. Click Test my design to watch the paired trials and compare modeled accidents. Conditions stay fixed for a fair comparison. Under 150 rounds, the preview runs quickly; longer runs illustrate 30 rounds. Every round is calculated, and playback returns to normal afterward.';
