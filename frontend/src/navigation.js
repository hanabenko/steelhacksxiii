...
export function createNavigation({orthographic,controls,element,getOrigin,onCamera,onMode}){
  ...
  let mode='orbit',camera=orthographic,yaw=0,pitch=0,look=null,wheelTravel=0;
  ...
  function setMode(next){
    keys.clear();held.clear();look=null;wheelTravel=0;
    if(next==='street'){
      ...
      camera=orthographic;controls.enabled=true;controls.mouseButtons.LEFT=next==='pan'?THREE.MOUSE.PAN:THREE.MOUSE.ROTATE;controls.mouseButtons.RIGHT=next==='pan'?THREE.MOUSE.ROTATE:THREE.MOUSE.PAN;controls.touches.ONE=next==='pan'?THREE.TOUCH.PAN:THREE.TOUCH.ROTATE;
    }
  ...
  function move(forward,right,up,amount,normalize=true){
    ...
    const offset=direction.multiplyScalar(forward).addScaledVector(side,right).add(new THREE.Vector3(0,up,0));if(offset.lengthSq()>1&&normalize)offset.normalize();offset.multiplyScalar(amount);
  ...
  const clear=()=>{keys.clear();held.clear();look=null;wheelTravel=0;};listen(window,'blur',clear);listen(document,'visibilitychange',clear);
  listen(element,'contextmenu',event=>event.preventDefault());
  listen(element,'pointerdown',event=>{if(mode!=='street'||![0,2].includes(event.button))return;look={id:event.pointerId,button:event.button,x:event.clientX,y:event.clientY};element.setPointerCapture(event.pointerId);});
  listen(element,'pointermove',event=>{if(mode!=='street'||!look||look.id!==event.pointerId)return;if(look.button===2){move((event.clientY-look.y),-(event.clientX-look.x),0,.12,false);look.x=event.clientX;look.y=event.clientY;return;}yaw-=(event.clientX-look.x)*.004;pitch=THREE.MathUtils.clamp(pitch-(event.clientY-look.y)*.004,-1.4,1.4);street.rotation.set(pitch,yaw,0,'YXZ');look.x=event.clientX;look.y=event.clientY;});
