export function installSumoStudy({services,container,getSettings,getScene,isBusy,onStart,onFinish}) {
 ...
 panel.innerHTML=`<summary>SUMO · Forbes / Bigelow signal study</summary><p>Compare original signals against your green-phase setting at this junction using the demand slider. Campus upgrades, weather, and game scores belong to the local model and are not applied here.</p><p>3 paired trials × 10 simulated minutes. Geometry: OpenStreetMap. Default traffic calibration comes from Fifth / Meyran; slider demand is your assumption.</p><button type="button" data-run-sumo>Run SUMO study</button><button type="button" data-stop-replay hidden>Return to campus traffic</button><p role="status" data-sumo-status></p><div data-sumo-results></div>`;
 ...
 run.onclick=async()=>{if(isBusy())return;run.disabled=true;onStart();status.textContent='Running SUMO with matched seeds…';results.replaceChildren();try{
   const settings=getSettings();const value=await services.replay({schemaVersion:2,intersection:'pitt-forbes-bigelow',seed:42,duration_s:600,settings:{demand:settings.demand,runs:3,av:0},upgrades:[{type:'signal_timing',main_green_s:settings.green,side_green_s:settings.green}]});
   ...
   for(const [key,label] of [['mean_speed_mph','Mean speed (mph)'],['average_delay_s_per_vehicle','Delay (seconds/vehicle)'],['throughput_vehicles_per_hour','Throughput (vehicles/hour)']]){
     const line=document.createElement('p'),before=value.baseline.metrics[key]?.mean,after=value.modified.metrics[key]?.mean;line.textContent=`${label}: ${Number.isFinite(before)?before.toFixed(1):'unavailable'} → ${Number.isFinite(after)?after.toFixed(1):'unavailable'}`;results.append(line);
   }
